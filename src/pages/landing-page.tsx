...
const schemaCards = [
  {
    name: "users",
    relation: "Created and managed by admins",
    fields: ["id uuid PK", "email unique", "full_name", "role", "created_at"]
  },
  {
    name: "assets",
    relation: "Assigned to users and linked to categories",
    fields: [
      "id uuid PK",
      "asset_tag unique",
      "name",
  ...
  {
    name: "categories",
    relation: "Defines reusable asset groupings",
...
const endpointCards = [
  {
    method: "GET",
    endpoint: "/rest/v1/assets?status=eq.available",
    summary: "List assets ready for assignment."
  },
  {
    method: "GET",
    endpoint: "/rest/v1/assets?select=*,category:categories(*),user:users(*)",
    summary: "Fetch inventory with related category and ownership data."
  },
  {
    method: "POST",
  ...
  {
    method: "PATCH",
    endpoint: "/rest/v1/assets?id=eq.{asset_id}",
    summary: "Update lifecycle status, location, or metadata."
  },
  {
    method: "POST",
  ...
  },
  {
    ...
    summary: "Admin-only employee and access directory."
  }
] as const;

const accessRules = [
  "Employees can view assets assigned to them and their related history.",
  "Admins and managers can create, update, assign, retire, and maintain records.",
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("assets")
        .select("*, categories(name), assigned_to(full_name)")
          ...
          <div className="arch-reveal space-y-8">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              {[
                ...
                { label: "Core tables", value: "5", detail: "Users, assets, logs, categories" },
