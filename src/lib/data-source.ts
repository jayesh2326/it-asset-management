...
async function invokeEdgeFunction(
  functionName: "admin-invite-user" | "admin-update-user-role",
  body: object
) {
  const client = requireSupabase();
  const {
  ...
  const functionUrl = `${runtimeConfig.supabaseUrl.replace(/\/$/, "")}/functions/v1/${functionName}`;
  ...
  if (!response.ok) {
    throw new Error(
      payload?.error ?? payload?.message ?? `Unable to call ${functionName}.`
    );
  }
...
function createSupabaseRepository(currentProfile: Profile | null): Repository {
  const repository: Repository = {
    ...
    async inviteUser(input: InviteUserInput) {
      ...
      await invokeEdgeFunction("admin-invite-user", input);
    },
    ...
    async updateUserRole(userId: string, role: Profile["role"]) {
      ...
      await invokeEdgeFunction("admin-update-user-role", { userId, role });
    }
  };
